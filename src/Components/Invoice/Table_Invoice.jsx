  ...
  const columns = [
    ...
    },
    {
      ...
      cell: row => <div className="d-flex align-items-center gap-2">
        ...
        <Link to={`/invoice-update/${row._id}`} className="btn btn-outline-success rounded-0 btn-sm"><BiEditAlt /></Link>
        {row.status === 'paid' || row.current_due > 0 ? (<button type="button" onClick={() => deleteInvoice(row._id)} className="btn btn-outline-danger rounded-0 btn-sm"><BiTrash /></button>) : (<button type="button" className="btn btn-dark rounded-0 btn-sm" disabled><BiTrash /></button>)}
      </div>,
      width: "150px"
    }
  ];
