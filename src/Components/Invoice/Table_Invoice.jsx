  ...
  const columns = [
    ...
    },
    {
      ...
      cell: row => <div className="d-flex align-items-center gap-2">
        ...
        {row.status === 'paid' && row.current_due < 1 ? (<button type="button" onClick={() => deleteInvoice(row._id)} className="btn btn-outline-danger rounded-0 btn-sm"><BiTrash /></button>) : (<button type="button" className="btn btn-dark rounded-0 btn-sm" disabled><BiTrash /></button>)}
      </div>,
      width: "150px"
    }
  ];
